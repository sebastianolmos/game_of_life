...
path = "GameOfLifeCUDA2/GameOfLifeCUDA2/x64/Release/"

def getValueFromProgram(width, height, iterations, threads) -> float:
    p = Popen(['GameOfLifeCUDA2.exe',  str(width), str(height), str(iterations), str(threads)], cwd=path, shell=True, stdout=PIPE, stdin=PIPE)
...
if __name__ == "__main__":
    ...
    with open('Data/cuda_128tpb_if.csv', mode='w') as csv_file:
