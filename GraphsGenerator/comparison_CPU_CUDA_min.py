"""Grafico con los valores obtenidos en la implementacion serial en CPU y CUDA"""
...
if __name__ == "__main__":
    ...
    with open(path + 'cuda_8tpb_min.csv', mode='r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        line_count = 0
        for row in csv_reader:
    ...
    with open(path + 'cuda_16tpb_min.csv', mode='r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        line_count = 0
        for row in csv_reader:
    ...
    with open(path + 'cuda_32tpb_min.csv', mode='r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        line_count = 0
        for row in csv_reader:
    ...
    ax.set(xlabel='Tamaño del mundo [Células]', ylabel='Células evaluadas por segundo [Millones]',
       title='Comparación de células evaluadas por segundo entre implementaciones serial en CPU\ny paralela en CUDA [Vista ampliada]')
    ax.grid()
