...
if __name__ == "__main__":
    ...
    with open(path + 'opencl_256tpb_if.csv', mode='r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        line_count = 0
        for row in csv_reader:
            tmp_time = float(row['time']) * 0.001
    ...
    line1 = ax.plot(n_size, n_eval1, 'r-o', label='OpenCL')
    line2 = ax.plot(n_size, n_eval2, 'b-o', label='OpenCL con ifs')
    line3 = ax.plot(n_size2, n_eval3, 'g-o', label='OpenCL no múltiplo de 32')

    ax.set(xlabel='Tamaño del mundo [Células]', ylabel='Speedup [n-veces]',
       title='Speedup para implementación en OpenCL y sus variaciones sobre CPU')
    ax.grid()

    ax.legend()
    fig.savefig("images/CPU_OpenCL_variations_speedup.png")
    plt.show()