...
if __name__ == "__main__":
    ...
    with open(path + 'cuda_9tpb.csv', mode='r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        line_count = 0
        for row in csv_reader:
    ...
    with open(path + 'cuda_27tpb.csv', mode='r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        line_count = 0
        for row in csv_reader:
            tmp_size = int(row['width']) * int(row['height']) 
    ...
    with open(path + 'cuda_243tpb.csv', mode='r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        line_count = 0
        for row in csv_reader:
            tmp_size = int(row['width']) * int(row['height']) 
    ...
    with open(path + 'cuda_729tpb.csv', mode='r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
        line_count = 0
        for row in csv_reader:
            tmp_size = int(row['width']) * int(row['height']) 
    ...
    ax.set(xlabel='Tamaño del mundo [Células]', ylabel='Células evaluadas por segundo [Millones]',
       title='Comparación de células evaluadas por segundo entre implementaciones\ncon threads por bloque (tpb) no múltiplo de 32 para CUDA')
    ax.grid()

    ax.legend()
    fig.savefig("images/CUDA_non32tpb.png")
    plt.show()